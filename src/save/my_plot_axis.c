...
void my_plot_axis(my_plot_t *plt)
{
    ...
    sfVertex line[] = {
        {{0, plt->graph->shift.y + tmp_vec.y / 2}, plt->graph->theme->axis},
        {{tmp_vec.x, plt->graph->shift.y + tmp_vec.y / 2}, plt->graph->theme->axis}
    };
    if (my_plot_is_onscreen(plt, line[0].position, axis))
        sfRenderWindow_drawPrimitives(plt->window, line, 2, sfLines, NULL);

    sfVertex line2[] = {
        {{tmp_vec.x / 2 + plt->graph->shift.x, 0}, plt->graph->theme->axis},
        {{tmp_vec.x / 2 + plt->graph->shift.x, tmp_vec.y}, plt->graph->theme->axis}
    };
